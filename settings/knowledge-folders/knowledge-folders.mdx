...
In the chat and assistants, you can add up to 20 files and send them with your prompt to the model. This is due to the limited amount of information the models can process simultaneously (= Context window, see [this guide](/resources/faq#context-window) for details about sending documents in the context window).
...
To push files to the knowledge folder, you can use the API of the folder. You can request an API key from [support@langdock.com](mailto:support@langdock.com). You can find the API schema in the [following guides](/api/knowledge-folder). The current rate limit is 50 requests per minute.