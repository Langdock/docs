...
<Info>
  You can use tools like [transform.tools](https://transform.tools/json-to-json-schema) or
  [easy-json-schema](https://easy-json-schema.github.io/) to generate JSON Schemas from example
  JSON objects.
</Info>
  ...
  const response = await axios.post(
    "https://api.langdock.com/assistant/v1/chat/completions",
    {
      ...
      messages: [
        {
          role: "user",
          content: "How would you rate this review: 'This product exceeded my expectations!'",
        },
      ],
      output: {
        type: "enum",
