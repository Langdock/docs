...
![](/images/prompt-library-2.png)

When using the prompt in a chat, the user is asked to fill out the field with the value in the individual situation.

<Tip>Advanced tip: You can combine assistants with prompts from the prompt library. If you have an assistant where you always follow a series of steps, you can save the steps in the library and then execute the steps one by one in the assistant chat.</Tip>