---
title: 'Actions'
description: 'Assistant Actions allow users to connect external tools and perform API calls to retrieve information and send requests. This can be used to create entries in other tools.'
---
...
OpenAPI is a specific standard format for calling APIs and describing the API parameters. Link: https://swagger.io/specification/
We have created an assistant, which you can find here. Simply paste the API documentation into the chat field, and the assistant will build the Open API schema for this endpoint.
...
2. The assistant calls the correct API but uses incorrect parameters
- Improve the descriptions of the parameters in the action description

3. The assistant is not working, but I don’t have an error
