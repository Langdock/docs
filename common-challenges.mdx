...
description: 'Common challenges when working with LLMs.'
---


We are used to deterministic software (if you press a button, the same specific action always happens). With stochastic interfaces (the results when using an AI model are always different), it can sometimes be difficult to get the desired result.


We collected some common issues to help you get the result you expect and improve your prompting. We are also always happy help you individually: thomas@langdock.com


## You are unhappy with the Response
If you are unhappy with the quality of the response, try to regenerate it by clicking on the circle arrow below the response (appears when hovering over the response). If this does not help, try editing the prompt by clicking on the pen below the prompt (also appears when hovereing over the prompt). Now you can provide more context what you expect from the AI model (we provided some ways to improve the prompt in our prompting [guide](/prompting))


## AI model replies to an email instead of improving it


**Solution:** ask to improve this email. Give clear instructions on what should be improved (tone, grammar, readability, …).
...
**Solution:** Maybe the document/integration has not been fully imported yet. You can check the status of the files in the list. “Imported” means that the import of the document is complete. Large integrations (Slack accounts with many users, channels and messages; large team drives; complex Notion spaces could take a moment to load). If you run into issues here or it takes a long time, please contact the Langdock team (thomas@langdock.com).
...
Only chats in web or plain mode can be shared at this point. If the chat can not be shared, the chat is or has been at one point been in a different mode.
